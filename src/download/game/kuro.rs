...
use crate::utils::{
    KuroIndex, KuroResource, SpeedTracker, extract_archive, move_all, validate_checksum,
};
...
impl Kuro for Game {
        ...
        F: Fn(u64, u64, u64, u64) + Send + Sync + 'static,
    {
        ...
        if dll.is_ok() {
            let mut f = tokio::fs::File::open(dlp.join("manifest.json").as_path())
                .await
                .unwrap();
        ...
        F: Fn(u64, u64, u64, u64) + Send + Sync + 'static,
    {
        ...
        if dll.is_ok() {
            let mut f = tokio::fs::File::open(p.join("manifest.json").as_path())
                .await
                .unwrap();
            let mut reader = String::new();
            f.read_to_string(&mut reader).await.unwrap();
            ...
            if preloaded {
                ...
                if files.patch_infos.is_some() {
                    ...
                    for d in diffs {
                        ...
                        let mut krdiff =
                            KrDiff::new(game_path.clone(), stringed, Some(stgs.clone()));
