...
impl Extras {
    pub async fn download_fps_unlock(
        repository: String,
        dest: String,
        ...
        let d = Path::new(&dest);
        if d.exists() {
            let rel = tokio::task::spawn_blocking(move || get_github_release(repository.clone()))
                .await
                .unwrap();
            if rel.is_some() {
                let r = rel.unwrap();
                ...
                if da.is_ok() {
                    ...
                    let dl = du
                        .download(d.join("fpsunlock.exe").as_path(), progress)
                        .await;
    ...
    pub async fn download_jadeite(
        ...
        let d = Path::new(&dest);
        if d.exists() {
            let rel = tokio::task::spawn_blocking(move || get_codeberg_release(repository.clone()))
                .await
                .unwrap();
            if rel.is_some() {
                let r = rel.unwrap();
                    ...
                    if dl.is_ok() {
                        if with_loader {
                            let r1 = tokio::task::spawn_blocking(move || {
                                get_github_release("TwintailTeam/3dmloader-Package".to_string())
                            })
                            .await
                            .unwrap();
                            if r1.is_some() {
    ...
    pub async fn download_xxmi_packages(
        gimi_repo: String,
        srmi_repo: String,
        zzmi_repo: String,
        wwmi_repo: String,
        ...
        let d = Path::new(&dest);
        if d.exists() {
            let gimi = tokio::task::spawn_blocking(move || get_github_release(gimi_repo.clone()))
                .await
                .unwrap();
            let srmi = tokio::task::spawn_blocking(move || get_github_release(srmi_repo.clone()))
                .await
                .unwrap();
            let zzmi = tokio::task::spawn_blocking(move || get_github_release(zzmi_repo.clone()))
                .await
                .unwrap();
            let wwmi = tokio::task::spawn_blocking(move || get_github_release(wwmi_repo.clone()))
                .await
                .unwrap();
            let himi = tokio::task::spawn_blocking(move || get_github_release(himi_repo.clone()))
                .await
                .unwrap();
                ...
                && himi.is_some()
            {
                ...
                if dg.is_ok() {
                    ...
                    let dl = du
                        .download(d.join("gimi.zip").as_path(), |_c, _t, _ns, _ds| {})
                        .await;
                ...
                if dg.is_ok() {
                    let mut du = dg.unwrap();
                    let dl = du
                        .download(d.join("srmi.zip").as_path(), |_c, _t, _ns, _ds| {})
                        .await;
                    status.push(dl.is_ok());
                }
                ...
                if dg.is_ok() {
                    let mut du = dg.unwrap();
                    let dl = du
                        .download(d.join("zzmi.zip").as_path(), |_c, _t, _ns, _ds| {})
                        .await;
                    status.push(dl.is_ok());
                }
                ...
                if dg.is_ok() {
                    let mut du = dg.unwrap();
                    let dl = du
                        .download(d.join("wwmi.zip").as_path(), |_c, _t, _ns, _ds| {})
                        .await;
                    status.push(dl.is_ok());
                }
                ...
                if dg.is_ok() {
                    let mut du = dg.unwrap();
                    let dl = du
                        .download(d.join("himi.zip").as_path(), |_c, _t, _ns, _ds| {})
                        .await;
                    status.push(dl.is_ok());
                }
                status.iter().all(|&b| b)
